...
uniform float u_warp_amplitude;         // noise amplitude
...
vec2 fbm_warp( vec3 pos, int octaves, float octavesFract, float frequency, float amplitude, float lacunarity, float gain,
               float turbulence ) {
...
vec2 fbm_warp_none( vec3 pos, int octaves, float octavesFract, float frequency, float amplitude, float lacunarity, float gain,
                    float turbulence )
{
	return vec2( 0 );
}
...
void main() { 
	...
	vec2 warp = FBM_WARP_FUN( vec3( pos, u_warp_anim ), u_warp_octaves, u_warp_octavesFract, u_warp_frequency * u_frequency, u_warp_amplitude, u_warp_lacunarity, u_warp_gain, u_warp_turbulence );
	pos += warp * 0.01;

	float n = fbm_noise( vec3( pos, u_noiseZ ), u_octaves, u_octavesFract, u_frequency, u_amplitude, u_lacunarity, u_gain, u_turbulence );
